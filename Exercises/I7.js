...
var Walker = function(x,y){
	...
	this.step2 = function(canvas){ // Perlin step function
		...
		var perlinX = noise.perlin2(this.perlinXSeed,this.perlinXSeed)*20;
		//var perlinY = noise.perlin2(this.perlinYSeed,this.perlinYSeed)*20;

		var randNum = Math.random();
		if (randNum < 0.25) { this.y -= perlinX; } 
		else if (randNum < 0.5) { this.y += perlinX; }
		else if (randNum < 0.75) { this.x -= perlinX; }
		else { this.x += perlinX; }
	}
};
