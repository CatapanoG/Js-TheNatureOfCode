...
// Particle
	...
	function Particle(x,y,fRadius) {
		this.radius = fRadius || 15;
		this.x = x;
		this.y = y;
		this.angle = 0;
		this.body; // defined below
...
// Car
	function Car(posX,posY){
		...
		this.b1 = new Box(posX,posY,false,60,20);
		this.p1 = new Particle(posX - 25, posY + 0, 19);
		this.p2 = new Particle(posX + 25, posY + 0, 15);
		this.joint1Def = new b2RevoJointDef();
		this.joint2Def = new b2RevoJointDef();
		this.joint1;
		this.joint2;

		this.joint1Def.Initialize(this.b1.body,this.p1.body,this.p1.body.GetPosition());
		this.joint1Def.enableMotor = true;
		this.joint1Def.motorSpeed = Math.PI*this.fAccFactor;
		this.joint1Def.maxMotorTorque = 10000.0;

		this.joint2Def.Initialize(this.b1.body,this.p2.body,this.p2.body.GetPosition());
		this.joint2Def.enableMotor = false;
		this.joint2Def.motorSpeed = Math.PI*this.fAccFactor;
		this.joint2Def.maxMotorTorque = 10000.0;

		this.joint1 = world.CreateJoint(this.joint1Def);
		this.joint2 = world.CreateJoint(this.joint2Def);
	};
	...
	Car.prototype = {
		...
		accelerate : function(){
			...
			this.joint1.SetMotorSpeed(Math.PI*this.fAccFactor);
			this.joint2.SetMotorSpeed(Math.PI*this.fAccFactor);
		},
		brake : function(){
			this.fAccFactor -= 0.1;
