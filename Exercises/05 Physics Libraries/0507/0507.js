...
//
// Exercise 5.7:
// Use a revolute joint for the wheels of a car. 
// Use motors so that the car drives autonomously. 
// Try using a chain shape for the road’s surface.
//
// Example-exercise ported-written by: Gennaro Catapano
...
function setup(context,canvas) {
	...
	for (var i = 0; i <= steps; i++) {
		verts.push(new b2Vec2(canvas.width*(i/steps),
							  Math.sin(Math.PI*(i/steps)*5 - Math.PI/2)*(canvas.height/4)*((steps-i)/steps)+(canvas.height/2)));
	};
...
// mouseHandler
	function mouseHandler(canvas){
		canvas.onmousedown = function(e){
			...
			cars.push(new Car(e.clientX,e.clientY));
		};
	};
//
