...
function setup(canvas) {
	...
	liquid = new Liquid(0,canvas.height/2,canvas.width,canvas.height/2,0.002);
	...
	for (var i = 0; i < moversLength; i++) {
		movers[i] = new Mover(
			...
			0);										// height linearly deceased 

		movers[i].side = Math.random()*50;
	};

	Mover.prototype.display = function(context){ // overriding the Moved obj display method
		context.fillStyle = "grey";
		context.fillRect(this.location.x,this.location.y,this.side,this.side);
	}; 
};
...
// Mover object (Js has no classes)
	...
	Mover.prototype = {
		...
		drag: function(liquid){
			...
			var dragMagnitude = liquid.c * this.side * speed * speed; // Added this.side
