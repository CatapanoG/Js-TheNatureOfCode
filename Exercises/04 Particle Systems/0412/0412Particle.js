...
Particle.prototype = {
	...
	display: function(context){
		...
		var iPartNumber;
		if (Math.abs(this.location.x - this.origin.x)<5) {
			iPartNumber = 1;
		} else if (Math.abs(this.location.x - this.origin.x)<10) {
			iPartNumber = 0;
		} else {
			iPartNumber = 2;
		};
		context.drawImage(oParticleImage[iPartNumber], 0, 0, fPartSize, fPartSize);
