...
//
// Exercise 4.3
// Make the origin point move dynamically. 
// Have the particles emit from the mouse location or use the concepts of velocity and acceleration to make the system move autonomously. 
//
...
function setup(context,canvas) {
	v2Mouse = new Vector2d(canvas.width/2,canvas.height/2);

	oParticleSystem = new ParticleSystem(new Vector2d(canvas.width/2,canvas.height/2));
};

function update(canvas){
	var _v2MouseDir = Vector2d.prototype.sub(v2Mouse,oParticleSystem.v2Origin);
	_v2MouseDir.normalize();
	oParticleSystem.applyForce(_v2MouseDir);

	oParticleSystem.update();
};

function draw(context,canvas) {
	background(context,canvas,"Black");

	oParticleSystem.draw(context);
};
...
// Particle object
	...
	function Particle(v2Location,iMass) {
