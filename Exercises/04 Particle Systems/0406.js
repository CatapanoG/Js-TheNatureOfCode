...
function update(canvas){
	for (var i = 0; i < aShapes.length; i++) {
		aShapes[i].checkClick();
		if (aShapes[i].shattered === true) {
			aPartSys.push(new ParticleSystem(aShapes[i].location,aShapes[i].width,aShapes[i].height));
			aShapes.splice(i,1);
			i--;
		};
	};

	for (var i = 0; i < aPartSys.length; i++) {
		aPartSys[i].update();
	};
};
...
function draw(context,canvas) {
	...
	for (var i = 0; i < aShapes.length; i++) {
		aShapes[i].display(context);
	};

	for (var i = 0; i < aPartSys.length; i++) {
		aPartSys[i].draw(context);
	};
};

// Shape object

	function RectShape(v2Location,iWidth,iHeight){
	...
	RectShape.prototype = {
		checkClick: function(){
			if (mouse.x >= this.location.x && mouse.x <= this.location.x + this.width &&
				mouse.y >= this.location.y && mouse.y <= this.location.y + this.height) {
				mouse = {};
				this.shattered = true;
			};
		},
		display: function(context){
			rectangle(context,this.location.x,this.location.y,this.width,this.height);
		}
	};
...
// Particle system object
	...
	function ParticleSystem(v2Location,iWidth,iHeight){
		this.aParticles = [];
		...
		for (var i = 0; i < this.iParticles; i++) {
			this.addParticle();
		};
	};
	...
	ParticleSystem.prototype = {
		addParticle : function(){
			this.aParticles.push(new Particle(new Vector2d(
				this.v2Origin.x + Math.random()*this.width, 
				this.v2Origin.y + Math.random()*this.height)));
		},
		update: function(){
...
// Particle object
	...
	function Particle(v2Location) {
		...
		this.velocity = new Vector2d((Math.random()-0.5)*3,(Math.random()-1)*3);
